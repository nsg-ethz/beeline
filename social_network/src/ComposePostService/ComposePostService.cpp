...
int main(int argc, char *argv[]) {
  ...
  std::string user_timeline_path = config_json["user-timeline-service"]["path"];
  ...
  std::string text_path = config_json["text-service"]["path"];
  ...
  std::string media_path = config_json["media-service"]["path"];
  ...
  std::string home_timeline_path = config_json["home-timeline-service"]["path"];
  ...
  std::string unique_id_path = config_json["unique-id-service"]["path"];
  ...
  ClientPool<ThriftClient<UserTimelineServiceClient>> user_timeline_client_pool(
      "user-timeline-client", user_timeline_addr, user_timeline_port, user_timeline_path, 0,
  ...
  ClientPool<ThriftClient<TextServiceClient>> text_client_pool(
      "text-service-client", text_addr, text_port, text_path, 0, text_conns, text_timeout,
      text_keepalive, config_json);
  ClientPool<ThriftClient<UserServiceClient>> user_client_pool(
      "user-service-client", user_addr, user_port, user_path, 0, user_conns, user_timeout,
      user_keepalive, config_json);
  ClientPool<ThriftClient<MediaServiceClient>> media_client_pool(
      "media-service-client", media_addr, media_port, media_path, 0, media_conns,
      media_timeout, media_keepalive, config_json);
  ClientPool<ThriftClient<HomeTimelineServiceClient>> home_timeline_client_pool(
      "home-timeline-service-client", home_timeline_addr, home_timeline_port, home_timeline_path, 0,
  ...
  ClientPool<ThriftClient<UniqueIdServiceClient>> unique_id_client_pool(
      "unique-id-service-client", unique_id_addr, unique_id_port, unique_id_path, 0,
