...
if [[ -z "${POLICY}" || ${POLICY} == "0" ]]; then
    if [[ -z "${COMPLEXITY}" || ${COMPLEXITY} == "1" ]]; then
        ...
        script/bench.sh sm -e "PROXY_CONFIG=ssm-p0-c1.yaml REPLICAS=${REPLICAS} FRONTEND_ARGS=${FRONTEND_ARGS}" -c docker/ssm-envoy.yaml -n ${NAME}-p0-c1 -p l4fp -s ${SCRIPT} -f ${FROM} -t ${TO} -m ${MONITOR}
    fi
    ...
    if [[ -z "${COMPLEXITY}" || ${COMPLEXITY} == "2" ]]; then
        ...
        script/bench.sh sm -e "PROXY_CONFIG=ssm-p0-c2.yaml REPLICAS=${REPLICAS} FRONTEND_ARGS=${FRONTEND_ARGS}" -c docker/ssm-envoy.yaml -n ${NAME}-p0-c2 -p l4fp -s ${SCRIPT} -f ${FROM} -t ${TO} -m ${MONITOR}
    fi
    ...
    if [[ -z "${COMPLEXITY}" || ${COMPLEXITY} == "3" ]]; then
        ...
        script/bench.sh sm -e "PROXY_CONFIG=ssm-p0-c3.yaml REPLICAS=${REPLICAS} FRONTEND_ARGS=${FRONTEND_ARGS}" -c docker/ssm-envoy.yaml -n ${NAME}-p0-c3 -p l4fp -s ${SCRIPT} -f ${FROM} -t ${TO} -m ${MONITOR}
    fi
fi
